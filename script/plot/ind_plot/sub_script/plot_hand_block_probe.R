...
plot_hand_blk <- function(roi){
  
  tmp_df <- subset(df_data_probe, run == roi)
  
  tmp_plot.pre <- ggplot(tmp_df, aes(x = prb_tri)) +
  ...
  tmp_plot <- format_gg(tmp_plot.pre, xlabel = "Trial", ylabel = "Hand Error (Tgt - Hand) [deg]", 
                     ptitle = sprintf("Run %d (%s)",roi, probe_label[roi]), pos.leg = "bl",
                     xlimit=(range(tmp_df$prb_tri)+c(-1,1)), ylimit = yrange_a, 
                     expand_coord = F)
  
}


plot_list <- plyr::alply(unique(df_data_probe$run), 1, plot_hand_blk)
plot_list_save = marrangeGrob(plot_list, nrow = nr, ncol = nc, layout_matrix = matrix(1:(nr*nc),nr,nc,T),
                              top = sprintf("Hand Profile in Probe %s", ind_tag),
                              left = "",
                              bottom = "") # convert the list of plots

save_plots(tgt_plot = plot_list_save, fname = sprintf("hand_block_probe"), pdf_only = T, readme_content = "")
