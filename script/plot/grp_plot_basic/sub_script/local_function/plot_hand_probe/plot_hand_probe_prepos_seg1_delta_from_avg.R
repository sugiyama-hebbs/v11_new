yrange_a <- c(-10,10)
local_fname_tag <- "seg1_delta_from_avg"
...
ref_tri_seg1 <- 9:11 # reference trial from whose average delta hand is computed. hard coding. This works only when prb_pre_null_inc = 10, so adjust this if you change it 

length_tag <- length(ref_tri_seg1)

# for checking
# df_data_probe_ind_seg1_ref <- df_data_probe_ind %>%
#   dplyr::filter(prb_tri %in% ref_tri_seg1, blk %in% c(main_blk[1],main_blk[length(main_blk)])) %>%
#   group_by(sub_id,blk) %>%
#   summarise(m = mean(herr, na.rm=T)) %>% 
#   ungroup()


df_data_probe_ind_seg1 <- df_data_probe_ind %>% 
  group_by(sub_id,blk) %>% 
  mutate(d_herr = herr - mean(herr[ref_tri_seg1],na.rm=T)) %>% 
  ungroup() %>% 
  dplyr::filter(prb_tri %in% prb_tri_seg1, blk %in% c(main_blk[1],main_blk[length(main_blk)])) %>% 
  dplyr::select(-herr) %>% 
  dplyr::rename(herr = d_herr)
...
save_plots(tgt_plot = plot_list_save, fname = sprintf("%s_%d",local_fname_tag, length_tag), 
...
save_plots(tgt_plot = plot_list_td_save, fname = sprintf("%s_%d_td",local_fname_tag, length_tag), 
           mdir = sub_mdir, sdir = sub_sdir,
           pdf_only = T, readme_content = "")
