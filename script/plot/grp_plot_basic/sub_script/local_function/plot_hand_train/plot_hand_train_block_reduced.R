...
local_fname_tag <- "block_reduced"

plot_hand_blk <- function(boi){
  
  tmp_df <- subset(df_data_train, blk == boi & (lead(rot,1) != 0 | lag(rot,1) != 0))
...
plot_hand_blk_td <- function(boi){
  ...
  tmp_df <- subset(df_data_train_td, blk == boi & (lead(rot,1) != 0 | lag(rot,1) != 0))
