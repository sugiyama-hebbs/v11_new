...
source(sprintf("script/plot/%s/sub_script/local_function/%s/func_summarise_train_dh.R",main_dir,sub_sdir))
...
df_data_train_dh <- df_data_train_dh_ind %>% 
  ...
  mutate(rot_dir = factor(ifelse(rot > 0, "+","-"), levels = c("+","-")))
...
plot_hand_blk <- function(boi){
  ...
  tmp_plot.pre <- ggplot(tmp_df, aes(x = trial, color = cond, group = cond:rot_dir)) +
    geom_hline(yintercept = 0, color="gray") +
    geom_line(aes(y=m, linetype = rot_dir), alpha = .8) +
...
plot_hand_blk_td <- function(boi){
  ...
  tmp_plot.pre <- ggplot(tmp_df, aes(x = trial, color = td, group = td:rot_dir)) +
    geom_hline(yintercept = 0, color="gray") +
    geom_line(aes(y=m, linetype = rot_dir), alpha = .8) +
    geom_point(aes(y=m), alpha = .8) +
...
### delta-hand
## 4groups
plot1.pre <- ggplot(df_data_train_dh, aes(x=blk, color = cond, group = cond:rot_dir)) +
  geom_hline(yintercept = 0, color = "gray") +
  geom_line(aes(y=m, linetype=rot_dir))+
  geom_point(aes(y=m))+
...
## Lrn vs NLrn
plot2.pre <- ggplot(df_data_train_dh_td, aes(x=blk, color = td, group = td:rot_dir)) +
  geom_hline(yintercept = 0, color = "gray") +
  geom_line(aes(y=m, linetype=rot_dir))+
  geom_point(aes(y=m))+
  geom_errorbar(aes(ymin=m-se, ymax=m+se), width = .3) +
