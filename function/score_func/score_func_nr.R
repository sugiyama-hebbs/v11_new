...
score_func_nr <- function(dh,rot){
  ...
  for (tri in 1:num_toi){
    ...
    # score[tri] <- ceiling(max_score - abs(adapt_ratio[tri])*10) + bonus_score
    score[tri] <- max_score - floor(abs(adapt_ratio[tri])*10) + bonus_score
