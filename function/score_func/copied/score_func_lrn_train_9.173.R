...
score_func_lrn <- function(dh,rot,count_s,difficulty){
  ...
  for (tri in 1:num_toi){
    ...
    if (adapt_ratio[tri] >= 1.0){
      score[tri] <- max_score - floor(abs(reward_zone*(adapt_ratio[tri]-1.0)*difficulty[tri])) + bonus_score
    } else {
      score[tri] <- max_score - floor(abs(reward_zone*(1.0 - adapt_ratio[tri])*difficulty[tri])) + bonus_score
    }
