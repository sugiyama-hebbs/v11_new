...
save_plots = function(tgt_plot = last_plot(), mdir = main_dir, sdir = sub_dir, fname = fig_name, readme_content = desc_note, pdf_only = F,
  ...
  if (png_only){
    fname_plot_png = sprintf("%s/png/%s.png",save_dir,fname)
    dir.create(file.path(sprintf("%s/%s",mdir_path,sdir), "png"), showWarnings = FALSE)
    ggsave(fname_plot_png, plot = tgt_plot, width = 20, height = 20, units = "cm", device = "png", dpi = "screen")    
